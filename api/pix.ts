...
// Importação compatível com CommonJS e Vercel:
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'initiate') {
        ...
        // Corrigido: este é o construtor correto no CommonJS/Node/Vercel
        const PixClass = qrcodePix.Pix || (qrcodePix as any).Pix;
        if (!PixClass) {
          return res.status(500).json({ error: "Biblioteca Pix não está disponível" });
        }

        const pix = new PixClass({
          key: key.replace(/\D/g, ''),
          name: "EMPRESA LTDA", // ajuste para sua empresa
          city: "SAO PAULO",   // ajuste para sua cidade
